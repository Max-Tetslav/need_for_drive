...
export const getGeocoderUrl = (searchPoint: string) => {
  return `https://geocode-maps.yandex.ru/1.x/?format=json&apikey=${GEOCODER_API}&geocode=${encodeURI(searchPoint)}}`;
};
...
export const getGeoJsonObjectList = (pointsList: IGeoPoint[]): GeoJSONSourceRaw => ({
  type: 'geojson',
  data: {
